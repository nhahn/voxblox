...
ProjectiveTsdfIntegrator<InterpolationScheme::kNearestNeighbor>::interpolate(
  ...
  return range_image(std::lround(h), std::lround(w));
}
...
ProjectiveTsdfIntegrator<InterpolationScheme::kMinNeighbor>::interpolate(
  ...
  if (h >= vertical_resolution_ || w >= horizontal_resolution_) {
    return range_image(static_cast<long>(std::floor(h)), static_cast<long>(std::floor(w)));
  }
...
ProjectiveTsdfIntegrator<InterpolationScheme::kBilinear>::interpolate(
  ...
  long h_lint = static_cast<long>(h_int), w_lint = static_cast<long>(w_int);
  const float left = (1 - h_decimals) * range_image(h_lint, w_lint) +
                     h_decimals * range_image(h_lint + 1, w_lint);
  const float right = (1 - h_decimals) * range_image(h_lint, w_lint + 1) +
                      h_decimals * range_image(h_lint + 1, w_lint + 1);
...
ProjectiveTsdfIntegrator<InterpolationScheme::kAdaptive>::interpolate(
  ...
  const float top_left = range_image(static_cast<long>(h_int), static_cast<long>(w_int));
  const float top_right =
      range_image(static_cast<long>(h_int), static_cast<long>(std::fmod(w_int + 1, horizontal_resolution_)));
  const float bottom_left = range_image(static_cast<long>(h_int + 1), static_cast<long>(w_int));
  const float bottom_right =
      range_image(static_cast<long>(h_int + 1), static_cast<long>(std::fmod(w_int + 1, horizontal_resolution_)));
